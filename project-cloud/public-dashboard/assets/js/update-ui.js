...
$(document).ready(() => {
	...
	webSocket.onmessage = function onMessage(message) {
		try {
			...
			// var today = new Date()
			// var currrentTime = today.toLocaleTimeString()
			// var timeDiff = currrentTime -  body.time
			// console.log("1. Time from msg:", messageData.MessageDate)
			// console.log("2. Time from Rpi:", body.time)
			// console.log("3. Time now on board:", currrentTime, today.getMilliseconds() )
			// console.log("4. Time diff:", timeDiff)
			switch (eventtype) {
				case BIC:
					// code block
		...
		$.notify(
			{
				...
				icon: 'fa fa-bullseye',
				message: 'Ball in cup! Team <b>' + convertTeamToName(team) + '</b> scores!' // Team  <b>' + convertTeamToName((team == TEAM_HOME) ? TEAM_AWAY : TEAM_HOME) + '</b> has ' + score + ' cups remaining.
			},
			{
				type: color,
				timer: 5000,
